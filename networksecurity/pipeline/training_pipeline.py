...
class TrainingPipeline:
  ...
  ## local artifact is going to s3 bucket     #lec 298
    def sync_artifact_dir_to_s3(self):
        try:
            aws_bucket_url = f"s3://{TRAINING_BUCKET_NAME}/artifact/{self.training_pipeline_config.timestamp}"
            self.s3_sync.sync_folder_to_s3(folder = self.training_pipeline_config.artifact_dir,aws_bucket_url=aws_bucket_url) #Artificats_timestamp
        except Exception as e:
            raise NetworkSecurityException(e,sys)
    ...
    def sync_saved_model_dir_to_s3(self):
        try:
            ...
            self.s3_sync.sync_folder_to_s3(folder = self.training_pipeline_config.model_dir,aws_bucket_url=aws_bucket_url) #model.dir= final_model folder
        except Exception as e:
            raise NetworkSecurityException(e,sys)
